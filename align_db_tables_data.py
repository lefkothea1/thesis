...
also plots mean sped of mouse and typoing per pp
...
for pp_idx in range(len(data)):
    ...
    for cond in [0,1]: 
        ...
        #----------------------------------------------------------------------
        # ------------------------------------Combining ALL mouse tables--------------------
        #----------------------------------------------------------------------
        allMouse_out = pd.concat([out_click, out_move, scroll_out],axis=1)
        # below would also work even if we dont have exact same timebins
        # combo_out = pd.merge(out_click, out_move, on='ts', how='outer')
 #----------------------------------------------------------------------
 #----------------------------------------------------------------------
        #----------------------------------------------------------------------
        # ------------------------------------keyboard--------------------
