...
should generate openFace features here on the temp df of each pp!
...
for file_name in glob.glob(dir_in + '*.csv'):
    ...
    name = file_name.split('\\')[-1].split('_')[0] #see if u need it, will split filename by _ and keep the 1st part (which should be pp#)
    x = pd.read_csv(file_name, sep=',')
    # x.columns = name + x.columns #addig prefix, prob dont need it here
    
    #finalising structure.....could wrap this in function
    #filtering out frame rows where tracking wasnt successful IF I DO THIS then i wont have equal nr of rows, problem in aggregation
    # x.drop(x.loc[x['success']==0].index, inplace=True)
    ...
    # cols2drop = [' face_id', 'frame', ' success', ' confidence', ' gaze_angle_x', ' gaze_angle_y']
    # x.drop(cols2drop, inplace=True, axis=1) not needed, will just select the useful oones
    ...
    # all_data = pd.concat([all_data,temp],axis=0) #2 get all pps in one file, dont want that currently
